...
import (
	...
	"github.com/6sl3pt/mcp-go-cli/pkg/history"
	"github.com/6sl3pt/mcp-go-cli/pkg/llm"
	"github.com/6sl3pt/mcp-go-cli/pkg/llm/ollama"
	"github.com/6sl3pt/mcp-go-cli/pkg/llm/openai"
	...
	for {
		...
		if err != nil {
			...
			if strings.Contains(err.Error(), "overloaded_error") {
				...
				log.Warn("Model is overloaded, backing off...",
					"attempt", retries+1,
	...
	for _, toolCall := range message.GetToolCalls() {
		...
		if toolResult.Content != nil {
			...
			for _, item := range toolResult.Content {
				...
				if err != nil {
					log.Error("Failed to marshal item", "error", err)
					continue
				}

				var contentMap map[string]interface{}
				if err := json.Unmarshal(jsonBytes, &contentMap); err != nil {
					log.Error("Failed to unmarshal item to map", "error", err)
					continue
				}
